...
--> statement-breakpoint
CREATE INDEX `recommendation_user_requested_idx` ON `recommendation` (`user_id`,`requested_at`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `measurement_user_measured_idx` ON `measurement` (`user_id`,`measured_at`);--> statement-breakpoint
CREATE TABLE `exercise_event` (
	...
	`difficulty` integer,
	`pain_flag` integer DEFAULT false NOT NULL,
	`notes` text,
	`created_at` text NOT NULL,
	FOREIGN KEY (`user_id`) REFERENCES `user_profile`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE INDEX `exercise_event_user_occurred_idx` ON `exercise_event` (`user_id`,`occurred_at`);